...
function report(dataDir, outDir) {
  const domainsFile           = `${dataDir}condensed.csv`;
  const modulesFile           = `${dataDir}uses.csv`;
  const remapFile             = `${dataDir}resolved_remap.json`;
  const modulesToKeywordsFile = `${dataDir}resolved_modulesToKeywords.json`;
  const mailmapFile           = `${dataDir}mailmap`;
