...
function modulesToKeywords(file, outBase, keywordStatsFile = process.cwd() + '/out/keywords_counted.json') {
  ...
  table = table.map(transformRow);

  const stringified = stringify(table);
