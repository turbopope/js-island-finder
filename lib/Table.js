  ...
  static parse(csv, overwrite = true) {
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (cells.length != headers.length) {
        throw new Error(`Row ${r}'s (${cells.length}) the header's length (${headers.length}).`);
      }
      ...
      for (let c = 1; c < headers.length; c++) {
        ...
        if (overwrite) {
          result.set(row_header, headers[c], cells[c]);
        } else {
          result.setOrAdd(row_header, headers[c], cells[c]);
        }
      }
    }
